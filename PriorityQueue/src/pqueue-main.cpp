...
int main() {
    ...
    while (true) {
        cout << endl;
        cout << "S) SLinked, U) USLinked, H) Heap, E)xtra, Q)uit" << endl;
...
void test(T& queue) {
    ...
    while (true) {
        ...
        if (choice.empty() || choice == "Q") {
            ...
            ttime.stop();
            cout << "The dequeuing of "<< count<< "  string  took " << ttime.elapsed() << "ms to finish." << endl;
 ...
 */
template <typename T>
void bulkEnqueue(T& queue, Timer& tim) {
    int count = getInteger("How many elements? ");
    ...
    if (choice2 == "R") {
        ...
        tim.stop();
        cout << "The enqueuing of "<< count<< "  string  took " << tim.elapsed() << "ms to finish." << endl;
